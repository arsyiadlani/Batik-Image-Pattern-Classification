...
# Loading Model
json_file = open('model_a_kembangan.json', 'r')
loaded_model_json_c_kembangan = json_file.read()
json_file.close()
loaded_model_c_kembangan = model_from_json(loaded_model_json_c_kembangan)


# load weights into new model
loaded_model_c_kembangan.load_weights("model_a_kembangan.h5")

folder_path = os.path.join(os.path.dirname(os.path.abspath("c_action.py")), "training_files_a")

train_dir_c_kembangan = folder_path

train_img_c_kembangan_generator = ImageDataGenerator(rescale = 1./255,
                                   validation_split = 0.2)  # Split dataset into training (80%) and validation (20%) dataset

val_img_c_kembangan_generator = ImageDataGenerator(rescale = 1./255,
                                   validation_split = 0.2)  # Split dataset into training (80%) and validation (20%) dataset

train_img_c_kembangan = train_img_c_kembangan_generator.flow_from_directory(
    train_dir_c_kembangan,
...
val_img_c_kembangan= val_img_c_kembangan_generator.flow_from_directory(
    train_dir_c_kembangan,
...
test_path= os.path.join(os.path.dirname(os.path.abspath("c_action.py")), "test_files")

test_dir_c_kembangan = test_path

i = 1

folder_path_test = os.path.join(os.path.dirname(os.path.abspath("c_action.py")), "test_files")
train_classes =list(train_img_c_kembangan.class_indices.keys())
...
for file_name in os.listdir(folder_path_test):
  ...
  cls = loaded_model_c_kembangan.predict(img_array)
...
print()
train_pred_c_kembangan = loaded_model_c_kembangan.predict(train_img_c_kembangan)
train_predictions_c_kembangan = np.argmax(train_pred_c_kembangan, axis=1)

cm_train_c_kembangan = metrics.confusion_matrix(train_predictions_c_kembangan, train_img_c_kembangan.classes)

evaluate(cm_train_c_kembangan)
print()

print('------')
print('Kinerja Proses Validation')
print()
val_pred_c_kembangan = loaded_model_c_kembangan.predict(val_img_c_kembangan)
val_predictions_c_kembangan = np.argmax(val_pred_c_kembangan, axis=1)

cm_val_c_kembangan = metrics.confusion_matrix(val_predictions_c_kembangan, val_img_c_kembangan.classes)

evaluate(cm_val_c_kembangan)
