...
# Loading Model
json_file = open('model_a_inception.json', 'r')
loaded_model_json_a_incep = json_file.read()
json_file.close()
loaded_model_a_incep = model_from_json(loaded_model_json_a_incep)


# load weights into new model
loaded_model_a_incep.load_weights("model_a_inception.h5")

folder_path = os.path.join(os.path.dirname(os.path.abspath("a_action.py")), "training_files_a")

train_dir_a_incep = folder_path
...
test_path= os.path.join(os.path.dirname(os.path.abspath("a_action.py")), "test_files")

test_dir_a_incep = test_path

i = 1

folder_path_test = os.path.join(os.path.dirname(os.path.abspath("a_action.py")), "test_files")
train_classes =list(train_img_a_incep.class_indices.keys())
...
for file_name in os.listdir(folder_path_test):
  ...
  cls = loaded_model_a_incep.predict(img_array)
...
print()
train_pred_a_incep = loaded_model_a_incep.predict(train_img_a_incep)
...
evaluate(cm_train_a_incep)
print()

print('------')
print('Kinerja Proses Validation')
print()
val_pred_a_incep = loaded_model_a_incep.predict(val_img_a_incep)
