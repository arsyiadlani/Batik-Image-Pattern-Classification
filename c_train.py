...
train_dir_c = os.path.join(os.path.dirname(os.path.abspath("c_train.py")), "training_files_a")

train_img_c_generator = ImageDataGenerator(rescale = 1./255,
                                   validation_split = 0.2)  # Split dataset into training (80%) and validation (20%) dataset

val_img_c_generator = ImageDataGenerator(rescale = 1./255,
                                   validation_split = 0.2)  # Split dataset into training (80%) and validation (20%) dataset

train_img_c = train_img_c_generator.flow_from_directory(
    train_dir_c,
...
val_img_c = val_img_c_generator.flow_from_directory(
    train_dir_c,
...
# Saving Model
model_json_a = model_kem.to_json()
with open("model_a_kembangan.json", "w") as json_file:
    json_file.write(model_json_a)

model_kem.save_weights("model_a_kembangan.h5") 
